...
# Read data
data_orig <- readRDS(file=file.path(datadir, "RAM_WC_recruitment_data.Rds")) 
...
data_out <- data1 %>% 
  ...
  filter(use=="yes") %>% 
  # Simplify
  dplyr::select(stockid:c)

# Export data
saveRDS(data_out, file=file.path(datadir, "RAM_WC_recruitment_data_trimmed.Rds"))
