###Formating ODA NEws releases data by species
...
# Read data
data_orig <- readr::read_csv(file.path(datadir, "ODA_crabs_closure.csv"))
# Read season key
season_key_dcrab <- readxl::read_excel(file.path(datadir, "dcrab_season_key.xlsx"))
...
##Assumptions
##RAZOR CLAM
##1. If fishery is not clearly specified raw data ="not specified" and assumed is recreational in the code. 
##2. If it says "all" raw data category will be "all" and we will interpret as recreational and commercial in the code
...
data1 <- data_orig %>% 
  ...
  mutate(year=as.numeric(year),
         ##Mussels and razor clam
         fishery = case_when(fishery == "all" ~ "commercial/recreational",
                             fishery == "not specified" ~ "recreational",
                             T ~ fishery),
         ##Crabs
...
# Step 3. One row per area (beaches, bay)
## Skip this for razor clam and mussels step given than information is not clear
...
dun_comm <- build_closure_grid(data = data4, species ="Dungeness crab", fishery = "Commecial", region = "Ocean")
