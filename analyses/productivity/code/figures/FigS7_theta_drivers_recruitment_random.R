...
prod_thetas <- data_orig %>% 
  filter(prod_type=="Recruitment" & model_type!="Correlation") %>% 
...
# Plot 
g1 <- ggplot(stock_key, aes(x=bbmsy_rec, y=theta_random)) +
  # LM fit
...
# Plot 
g2 <- ggplot(stock_key, aes(x=ffmsy_rec, y=theta_random)) +
  # LM fit
  geom_smooth(method="lm", fill="grey80", color="black") +
  # Reference lines
  geom_hline(yintercept=0) +
...
# Plot 
g3 <- ggplot(stock_key, aes(x=sst_c_trend_rec*10, y=theta_random)) +
  # LM fit
  geom_smooth(method="lm", fill="grey80", color="black") +
  # Reference lines
  geom_hline(yintercept=0) +
...
# Plot 
g4 <- ggplot(stock_key, aes(x=linf_cm, y=theta_random)) +
  # LM fit
  geom_smooth(method="lm", fill="grey80", color="black") +
  # Reference lines
  geom_hline(yintercept=0) +
...
# Plot 
g5 <- ggplot(stock_key, aes(x=k, y=theta_random)) +
  # LM fit
  geom_smooth(method="lm", fill="grey80", color="black") +
  # Reference lines
  geom_hline(yintercept=0) +
...
# Plot 
g6 <- ggplot(stock_key, aes(x=tmax_yr, y=theta_random)) +
  # LM fit
  geom_smooth(method="lm", fill="grey80", color="black") +
  # Reference lines
  geom_hline(yintercept=0) +
...
lmfit <- lm(theta_random ~ tmax_yr, stock_key)
summary(lmfit)
